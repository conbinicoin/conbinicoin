...
ConbiniCoin has been subjected to a formal third-party security review. For security
announcements, audit results and other general security information, see
https://conbinicoin.com/support/security.html
...
This implementation of ConbiniCoin is not resistant to side-channel attacks. You
should assume (even unprivileged) users who are running on the hardware, or who
are physically near the hardware, that your `conbinicoind` process is running on will
be able to:
...
unprivileged) on the hardware your `conbinicoind` process runs on until these
